...
export default function ItemCard(props){
    const [items,setItems] = useState(Object.keys(props.menuItems[props.items])[0])
    const handleSubmit = (event) => {
        event.preventDefault();
        var itemName = props.items
        var itemSize = items
        var itemPrice = props.menuItems[props.items][items]
    ...
    return (
        <Form className='row mx-auto' onSubmit={handleSubmit}>
            <Form.Select aria-label="" className="col-md-6 w-50" value={items} onChange={handleChange}>
                {Object.keys(props.menuItems[props.items]).map((size) => {
                    return (
                        <option key={props.items+size} value={size}>{size} | ${props.menuItems[props.items][size]}</option>
                    )
                })}
            </Form.Select>
